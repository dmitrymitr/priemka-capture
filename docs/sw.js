const CACHE='priemka-shell-v1';
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(c=>c.addAll(['./','./index.html','./style.css','./app.js','./zip.js','./icon.svg','./manifest.webmanifest'])).then(()=>self.skipWaiting())));
